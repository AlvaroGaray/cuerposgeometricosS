package SwitchDeFiguras;
import java.util.*;
public class ElegirFigura {

	static Scanner es =new Scanner(System.in);
	 static double radio,RadioM, h,Perimetrobase,AreaLateral,Areabase,AreaTotal,Volumen, Generatriz, Caras,Vertices, Apotema, Aristas, Area, Areapentagonal;
	...
	private static void zonaesfera(Double radio,double h, double RadioM) {
		...
		double Volumenzonaesferica=(Math.PI*h)*(Math.pow(h, 2)+(3*Math.pow(RadioM, 2)+(3*Math.pow(radio, 2))));
